...
export function renderStudentDashboard(container, activeTab, triggerToast) {
  ...
  if (!student || student.role !== 'student') {
    container.innerHTML = '<div class="empty-state"><div class="empty-icon"></div><h3>No active student session found.</h3></div>';
    return;
  }
          ...
          <div class="flex justify-between items-start flex-wrap mt-3 gap-4">
            <div style="flex: 1; min-width: 250px;">
              ...
              <p class="text-xs text-muted mt-2">
                 ${student.city}, India • <a href="#" id="lnk-contact-info" class="text-primary font-semibold hover-underline" style="text-decoration: none;">Contact info</a>
              </p>
            </div>
          ...
          <div class="card" style="padding: var(--space-4); border-radius: var(--radius-2xl);">
            ...
            <div class="flex flex-col gap-3">
              ${store.state.communications.filter(c => c.toStudentId === student.id).map(c => {
                ...
                const colName = col ? col.name : 'Unknown College';
                return `
                  <div class="flex items-center gap-3 justify-between" style="border-block-end: 1px solid var(--color-border); padding-block-end: var(--space-2); &:last-child { border-block-end: none; margin-block-end: 0; }">
                    <div class="flex items-center gap-2" style="min-width: 0; flex: 1;">
                      <span style="font-size: 1.25rem; flex-shrink: 0;"></span>
                      <div style="min-width: 0; flex: 1;">
                        <h4 class="text-xs font-bold truncate" style="margin: 0;">${colName}</h4>
      ...
      btn.addEventListener('click', (e) => {
        ...
        if (matchedCollege) {
          ...
          let improvementsHTML = improvements.length > 0 
            ? improvements.map(i => `<li class="text-xs text-warning" style="margin-inline-start: var(--space-4);"> ${i}</li>`).join('')
            ...
            <div class="card-body">
              ...
              ` : `
                <div class="verified-document-pill pending mt-4">
                  <span></span> Self-reported (No doc)
                </div>
              `}
            </div>
          </div>
    ...
    btn.addEventListener('click', () => {
      ...
      if (selectedCollege) {
        ...
        let improvementsHTML = selectedCollege.improvements.length > 0 
          ? selectedCollege.improvements.map(i => `<li class="text-xs text-warning" style="margin-inline-start: var(--space-4);"> ${i}</li>`).join('')
          : '<li class="text-xs text-success" style="margin-inline-start: var(--space-4);"> No suggestions needed! Excellent fit.</li>';
...
function renderInboxTab(container, student, triggerToast) {
  ...
  const renderActiveChat = (selectedCommId) => {
    ...
    const collegeId = college ? college.id : 'unknown';
    ...
    if (comm.status === 'pending') {
      chatContainer.innerHTML = `
        <div class="empty-state" style="padding: var(--space-8);">
          <div class="empty-icon"></div>
          <h3 class="mb-2">Contact Request from ${collegeName}</h3>
    ...
    if (comm.status === 'rejected') {
      chatContainer.innerHTML = `<div class="empty-state"><div class="empty-icon">✕</div><h3>Request Declined</h3><p>You declined communication with ${collegeName}.</p></div>`;
      return;
    }
    ...
    const messagesHTML = messages.length === 0 
      ...
      : messages.map(m => {
          ...
          const senderName = isMe ? student.name : collegeName;
          return `
    ...
    chatContainer.innerHTML = `
      <div class="chat-header">
        <h4 class="font-bold">${collegeName}</h4>
    ...
    chatContainer.querySelector('#chat-input-form').addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        const reps = store.state.employees.filter(e => e.collegeId === collegeId);
        const repName = reps.length > 0 ? reps[0].name : 'Admissions Support';
        store.sendMessage(comm.id, collegeId, `Hello! Thank you for the note. Our admissions representative, ${repName}, will follow up with details on application timelines and campus visits shortly!`);
              ...
              <button class="inbox-channel-btn btn-ghost text-start w-full ${isPending ? 'pending-dot' : ''}" data-id="${c.id}">
                <div class="flex flex-col gap-1 w-full">
                  <div class="flex justify-between items-center w-full">
                    <strong class="text-sm truncate" style="max-width: 180px;">${colName}</strong>
